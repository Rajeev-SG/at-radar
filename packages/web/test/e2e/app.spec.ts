...
test('timeline loads and API feeds work', async ({ page, request }) => {
  ...
  // Test JSON feed  
  const jsonRes = await request.get('http://127.0.0.1:8787/api/feeds/json');
  ...
  if (page1.items[0]?.event_id) {
    const detail = await request.get(`http://127.0.0.1:8787/api/events/${page1.items[0].event_id}`).then((r) => r.json() as any);
